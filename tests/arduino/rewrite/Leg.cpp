...
Leg::Leg(String n, int pin, WalkingModeEnum mode)
{
    name = n;
    trigger_pin = pin;
    status = Initialized;
...
void Leg::_sparkle_flash()
{
    ...
    for (int i = 0; i < PIXELS_PER_LEG; i++)
    {
        if ((i < lower_foot_border) || (i > upper_foot_border))
            _setPixel(i, COLORS[BLACK].r, COLORS[BLACK].g, COLORS[BLACK].b, 0x00);
        else
            _setPixel(i, LWConfigs.sparkle.footFlashColor.r, LWConfigs.sparkle.footFlashColor.g, LWConfigs.sparkle.footFlashColor.b, 0x00);
    }
    _setLightMode(Flash);
}
...
void Leg::pulse_pulse(unsigned long currentTime, unsigned long syncTime, int syncLength, int syncValue, bool changeColor)
{
    ...
    if (changeColor)
    {
        if (LWConfigs.pulse.randomColor)
        {
            ...
            _pulse_r = COLORS[color].r;
            _pulse_g = COLORS[color].g;
            _pulse_b = COLORS[color].b;
        }
        else
        {
            _pulse_r = LWConfigs.pulse.color.r;
            _pulse_g = LWConfigs.pulse.color.g;
            _pulse_b = LWConfigs.pulse.color.b;
        }
    }
    ...
    byte r = byte((float)_pulse_r * (brightness * brightness * brightness));
    byte g = byte((float)_pulse_g * (brightness * brightness * brightness));
    byte b = byte((float)_pulse_b * (brightness * brightness * brightness));
...
void Leg::_setPixel(int i, byte r, byte g, byte b, byte dimmer)
{
    if ((i == 0) && true)
    {
        Serial.print("_setPixel before:: "); Serial.print(r); Serial.print(" - "); Serial.print(g); Serial.print(" - "); Serial.print(b); Serial.print(" dim: "); Serial.println(dimmer);
    }

    if (r > dimmer)
        _pixels[i][0] = r - dimmer;
    else
        _pixels[i][0] = 0;

    if (g > dimmer)
        _pixels[i][1] = g - dimmer;
    else
        _pixels[i][1] = 0;

    if (b > dimmer)
        _pixels[i][2] = b - dimmer;
    else
        _pixels[i][2] = 0;

    if ((i == 0) && false)
    {
        Serial.print("_setPixel:: "); Serial.print(_pixels[i][0]); Serial.print(" - "); Serial.print(_pixels[i][1]); Serial.print(" - "); Serial.print(_pixels[i][2]); Serial.print(" dim: "); Serial.println(dimmer);
    }

    _pixels[i][0] = map(_pixels[i][0], 0, 255, 0, LWConfigs.main.maxBrightness);
    _pixels[i][1] = map(_pixels[i][1], 0, 255, 0, LWConfigs.main.maxBrightness);
    _pixels[i][2] = map(_pixels[i][2], 0, 255, 0, LWConfigs.main.maxBrightness);
